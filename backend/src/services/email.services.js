...
const sendPasswordResetEmail = async (email, code) => {
    try {
        // En development/testing: enviar al correo personal
        // En production: cambiar a tu dominio verificado (ej: noreply@tudominio.com)
        const fromEmail = process.env.NODE_ENV === 'production' 
            ? 'noreply@hospedajedigital.com' 
            : 'onboarding@resend.dev';
        
        const sendToEmail = process.env.NODE_ENV === 'production' 
            ? email 
            : process.env.RESEND_TEST_EMAIL || 'mic11.052006@gmail.com';
