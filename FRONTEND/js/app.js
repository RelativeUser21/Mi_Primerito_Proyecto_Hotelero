...
// ============================================
// FUNCIONES DE NAVEGACIÓN ENTRE SECCIONES (LEGACY SPA)
// ============================================
//
// Nota: Este archivo antes controlaba navegación tipo "SPA" dentro de index.html
// (mostrando/ocultando secciones). Ahora el sidebar global navega entre páginas
// usando `js/main.js` (window.cargarSeccion). Para no romper esa navegación,
// NO debemos declarar aquí una función global llamada `cargarSeccion`.

function cargarSeccionLegacySPA(seccion, event) {
    if (event) {
        event.preventDefault();
    }
...
const cargarServicioEnFormularioAdmin = (servicio) => {
    ...
    if (campoDuracion) {
        // Extraer solo números de la duración (en caso de que tenga formato como "60 min")
        const duracionTexto = servicio.Duracion || '';
        const duracionNumero = duracionTexto.toString().match(/\d+/)?.[0] || '';
        campoDuracion.value = duracionNumero;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.hash === '#seccion-administrar-habitaciones' || window.location.hash === '#seccion-administrar-servicios' || window.location.hash === '#seccion-administrar-clientes') {
    ...
    // Inicializar CRUD de habitaciones, servicios y clientes
...
const cargarRolesParaSelect = async () => {
    try {
        const respuesta = await window.apiRequest('/roles?page=1&limit=50');
        const roles = respuesta.data || respuesta || [];
        rolesDisponibles = Array.isArray(roles) ? roles : [];
        
        const select = document.getElementById('cliente-admin-idrol');
        if (!select) return;
        ...
        rolesDisponibles.forEach((rol) => {
            ...
            option.textContent = rol.NombreRol || rol.Nombre || `Rol ${rol.IDRol || rol.id}`;
...
const abrirModalClienteAdmin = async () => {
    cerrarModalesCRUD();
    await cargarRolesParaSelect();
...
const abrirDetalleClienteAdmin = (cliente) => {
    ...
    let nombreRol = 'Cliente'; // valor por defecto
    const rolId = Number(cliente.IDRol) || 2;
    const rol = rolesDisponibles.find(r => Number(r.IDRol || r.id) === rolId);
    if (rol) {
        nombreRol = rol.NombreRol || rol.Nombre || 'Cliente';
    }
    ...
    const contenido = `
        ...
        <div class="detalle-admin-grid detalle-admin-grid-servicio">
            <div class="detalle-admin-body detalle-admin-body-full">
                ...
                ${renderDetalleItem('Rol', escaparHtml(nombreRol))}
            </div>
        </div>
    `;
...
const cargarClienteEnFormularioAdmin = (cliente) => {
    ...
    if (campoRol) {
        const rolId = Number(cliente.IDRol) || 2; // Por defecto es Cliente (ID 2)
        campoRol.value = rolId;
    }
...
const limpiarFormularioClienteAdmin = (mostrarMensaje = true) => {
    ...
    if (campoRol) {
        // Establecer el rol por defecto a "Cliente" (ID 2)
        campoRol.value = 2;
    }
...
window.configurarClientesAdmin = configurarClientesAdmin;
