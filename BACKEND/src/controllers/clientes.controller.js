...
const getById = async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await db.query("SELECT * FROM clientes WHERE NroDocumento = ?", [id]);
        if (rows.length === 0) {
...
const create = async (req, res) => {
    ...
    try {
        ...
        const [clienteCreado] = await db.query("SELECT * FROM clientes WHERE NroDocumento = ?", [NroDocumento]);
...
const update = async (req, res) => {
    ...
    try {
        const [existing] = await db.query("SELECT * FROM clientes WHERE NroDocumento = ?", [id]);
